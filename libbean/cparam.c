...
/**
 * CParamSpec: (ref-func c_param_spec_ref_sink) (unref-func c_param_spec_unref) (set-value-func c_value_set_param) (get-value-func c_value_get_param)
 * @g_type_instance: private `GTypeInstance` portion
 * @name: name of this parameter: always an interned string
 * @flags: `CParamFlags` flags for this parameter
 * @value_type: the `GValue` type for this parameter
 * @owner_type: `GType` type that uses (introduces) this parameter
 *
 * `CParamSpec` encapsulates the metadata required to specify parameters, such as `GObject` properties.
 *
 * ## Parameter names
 *
 ...
 *
 * When creating and looking up a `CParamSpec`, either separator can be
...
#define	C_PARAM_USER_MASK			(~0U << C_PARAM_USER_SHIFT)
#define PSPEC_APPLIES_TO_VALUE(pspec, value)	(G_TYPE_CHECK_VALUE_TYPE ((value), C_PARAM_SPEC_VALUE_TYPE (pspec)))

/* --- prototypes --- */
static void	c_param_spec_class_base_init	 (CParamSpecClass	*class);
static void	c_param_spec_class_base_finalize (CParamSpecClass	*class);
static void	c_param_spec_class_init		 (CParamSpecClass	*class,
						  gpointer               class_data);
static void	c_param_spec_init		 (CParamSpec		*pspec,
						  CParamSpecClass	*class);
static void	c_param_spec_finalize		 (CParamSpec		*pspec);
...
static gint c_param_private_offset;

/* --- functions --- */
static inline CParamSpecPrivate *
c_param_spec_get_private (CParamSpec *pspec)
{
  return &G_STRUCT_MEMBER (CParamSpecPrivate, pspec, c_param_private_offset);
}

void
_c_param_type_init (void)
{
  ...
  const GTypeInfo param_spec_info = {
    sizeof (CParamSpecClass),

    (GBaseInitFunc) c_param_spec_class_base_init,
    (GBaseFinalizeFunc) c_param_spec_class_base_finalize,
    (GClassInitFunc) c_param_spec_class_init,
    (GClassFinalizeFunc) NULL,
    NULL,	/* class_data */

    sizeof (CParamSpec),
    0,		/* n_preallocs */
    (GInstanceInitFunc) c_param_spec_init,

    &param_value_table,
  };
  GType type;

  /* This should be registered as CParamSpec instead of CParam, for
   ...
   * depends on the "CParam" identifier, see #548689
   */
  type = g_type_register_fundamental (C_TYPE_PARAM, g_intern_static_string ("CParam"), &param_spec_info, &finfo, G_TYPE_FLAG_ABSTRACT);
  g_assert (type == C_TYPE_PARAM);
  c_param_private_offset = g_type_add_instance_private (type, sizeof (CParamSpecPrivate));
  g_value_register_transform_func (C_TYPE_PARAM, C_TYPE_PARAM, value_param_transform_value);
}

static void
c_param_spec_class_base_init (CParamSpecClass *class)
{
}

static void
c_param_spec_class_base_finalize (CParamSpecClass *class)
{
}

static void
c_param_spec_class_init (CParamSpecClass *class,
			 gpointer         class_data)
{
  class->value_type = G_TYPE_NONE;
  class->finalize = c_param_spec_finalize;
  class->value_set_default = NULL;
  class->value_validate = NULL;
  class->values_cmp = NULL;

  g_type_class_adjust_private_offset (class, &c_param_private_offset);
}

static void
c_param_spec_init (CParamSpec      *pspec,
		   CParamSpecClass *class)
{
  pspec->name = NULL;
  pspec->_nick = NULL;
...
static void
c_param_spec_finalize (CParamSpec *pspec)
{
  CParamSpecPrivate *priv = c_param_spec_get_private (pspec);
  ...
  if (!(pspec->flags & C_PARAM_STATIC_NICK))
    g_free (pspec->_nick);

  if (!(pspec->flags & C_PARAM_STATIC_BLURB))
    g_free (pspec->_blurb);
...
/**
 * c_param_spec_ref: (skip)
 * @pspec: (transfer none) (not nullable): a valid #CParamSpec
 *
 * Increments the reference count of @pspec.
 *
 * Returns: (transfer full) (not nullable): the #CParamSpec that was passed into this function
 */
CParamSpec*
c_param_spec_ref (CParamSpec *pspec)
{
  g_return_val_if_fail (C_IS_PARAM_SPEC (pspec), NULL);
...
/**
 * c_param_spec_unref: (skip)
 * @pspec: a valid #CParamSpec
 *
 * Decrements the reference count of a @pspec.
 */
void
c_param_spec_unref (CParamSpec *pspec)
{
  gboolean is_zero;

  g_return_if_fail (C_IS_PARAM_SPEC (pspec));

  is_zero = g_atomic_int_dec_and_test ((int *)&pspec->ref_count);

  if (G_UNLIKELY (is_zero))
    {
      C_PARAM_SPEC_GET_CLASS (pspec)->finalize (pspec);
    }
}
 ...
 * @pspec: a valid #CParamSpec
 *
 * The initial reference count of a newly created #CParamSpec is 1,
 * even though no one has explicitly called c_param_spec_ref() on it
 * yet. So the initial reference count is flagged as "floating", until
 * someone calls `c_param_spec_ref (pspec); c_param_spec_sink
 * (pspec);` in sequence on it, taking over the initial
 * reference count (thus ending up with a @pspec that has a reference
 * count of 1 still, but is not flagged "floating" anymore).
 */
void
c_param_spec_sink (CParamSpec *pspec)
{
  guintptr oldvalue;
  g_return_if_fail (C_IS_PARAM_SPEC (pspec));
  ...
  if (oldvalue & PARAM_FLOATING_FLAG)
    c_param_spec_unref (pspec);
}

/**
 * c_param_spec_ref_sink: (skip)
 * @pspec: a valid #CParamSpec
 *
 * Convenience function to ref and sink a #CParamSpec.
 *
 * Since: 2.10
 * Returns: (transfer full) (not nullable): the #CParamSpec that was passed into this function
 */
CParamSpec*
c_param_spec_ref_sink (CParamSpec *pspec)
{
  guintptr oldvalue;
  g_return_val_if_fail (C_IS_PARAM_SPEC (pspec), NULL);
  ...
  oldvalue = g_atomic_pointer_and (&pspec->qdata, ~(gsize)PARAM_FLOATING_FLAG);
  if (!(oldvalue & PARAM_FLOATING_FLAG))
    c_param_spec_ref (pspec);

  return pspec;
}

/**
 * c_param_spec_get_name:
 * @pspec: a valid #CParamSpec
 *
 * Get the name of a #CParamSpec.
 *
 ...
 */
const gchar *
c_param_spec_get_name (CParamSpec *pspec)
{
  g_return_val_if_fail (C_IS_PARAM_SPEC (pspec), NULL);
...
/**
 * c_param_spec_get_nick:
 * @pspec: a valid #CParamSpec
 *
 * Get the nickname of a #CParamSpec.
 *
 * Returns: the nickname of @pspec.
 */
const gchar *
c_param_spec_get_nick (CParamSpec *pspec)
{
  g_return_val_if_fail (C_IS_PARAM_SPEC (pspec), NULL);
  ...
  if (pspec->_nick)
    ...
    {
      CParamSpec *redirect_target;

      redirect_target = c_param_spec_get_redirect_target (pspec);
...
/**
 * c_param_spec_get_blurb:
 * @pspec: a valid #CParamSpec
 *
 * Get the short description of a #CParamSpec.
 *
 * Returns: (nullable): the short description of @pspec.
 */
const gchar *
c_param_spec_get_blurb (CParamSpec *pspec)
{
  g_return_val_if_fail (C_IS_PARAM_SPEC (pspec), NULL);
...
/**
 * c_param_spec_is_valid_name:
 * @name: the canonical name of the property
 *
 * Validate a property name for a #CParamSpec. This can be useful for
 ...
 */
gboolean
c_param_spec_is_valid_name (const gchar *name)
{
  const gchar *p;
...
/**
 * c_param_spec_internal: (skip)
 * @param_type: the #GType for the property; must be derived from %C_TYPE_PARAM
 * @name: the canonical name of the property
 * @nick: (nullable): the nickname of the property
 * @blurb: (nullable): a short description of the property
 * @flags: a combination of #CParamFlags
 *
 * Creates a new #CParamSpec instance.
 *
 * See [canonical parameter names][class@GObject.ParamSpec#parameter-names]
 * for details of the rules for @name. Names which violate these rules lead
 * to undefined behaviour.
 *
 * Beyond the name, #CParamSpecs have two more descriptive strings, the
 ...
 *     #CParamSpec instance, which is initially floating
 */
gpointer
c_param_spec_internal (GType        param_type,
		       const gchar *name,
		       const gchar *nick,
		       const gchar *blurb,
		       CParamFlags  flags)
{
  CParamSpec *pspec;
  CParamSpecPrivate *priv;
  
  g_return_val_if_fail (C_TYPE_IS_PARAM (param_type) && param_type != C_TYPE_PARAM, NULL);
  g_return_val_if_fail (name != NULL, NULL);
  g_return_val_if_fail (c_param_spec_is_valid_name (name), NULL);
  g_return_val_if_fail (!(flags & C_PARAM_STATIC_NAME) || is_canonical (name), NULL);
  
  pspec = (gpointer) g_type_create_instance (param_type);

  if (flags & C_PARAM_STATIC_NAME)
    {
      /* pspec->name is not freed if (flags & C_PARAM_STATIC_NAME) */
      pspec->name = (gchar *) g_intern_static_string (name);
      if (!is_canonical (pspec->name))
        g_warning ("C_PARAM_STATIC_NAME used with non-canonical pspec name: %s", pspec->name);
    }
  else
    {
  ...
  priv = c_param_spec_get_private (pspec);
  priv->name_quark = g_quark_from_string (pspec->name);

  if (flags & C_PARAM_STATIC_NICK)
    pspec->_nick = (gchar*) nick;
  else
    pspec->_nick = g_strdup (nick);

  if (flags & C_PARAM_STATIC_BLURB)
    pspec->_blurb = (gchar*) blurb;
  else
    pspec->_blurb = g_strdup (blurb);

  pspec->flags = (flags & C_PARAM_USER_MASK) | (flags & C_PARAM_MASK);
  
  return pspec;
}

/**
 * c_param_spec_get_qdata:
 * @pspec: a valid #CParamSpec
 * @quark: a #GQuark, naming the user data pointer
 *
 * Gets back user data pointers stored via c_param_spec_set_qdata().
 *
 * Returns: (transfer none) (nullable): the user data pointer set, or %NULL
 */
gpointer
c_param_spec_get_qdata (CParamSpec *pspec,
			GQuark      quark)
{
  g_return_val_if_fail (C_IS_PARAM_SPEC (pspec), NULL);
...
/**
 * c_param_spec_set_qdata:
 * @pspec: the #CParamSpec to set store a user data pointer
 * @quark: a #GQuark, naming the user data pointer
 * @data: (nullable): an opaque user data pointer
 *
 * Sets an opaque, named pointer on a #CParamSpec. The name is
 * specified through a #GQuark (retrieved e.g. via
 * g_quark_from_static_string()), and the pointer can be gotten back
 * from the @pspec with c_param_spec_get_qdata().  Setting a
 * previously set user data pointer, overrides (frees) the old pointer
 * set, using %NULL as pointer essentially removes the data stored.
 */
void
c_param_spec_set_qdata (CParamSpec *pspec,
			GQuark      quark,
			gpointer    data)
{
  g_return_if_fail (C_IS_PARAM_SPEC (pspec));
...
/**
 * c_param_spec_set_qdata_full: (skip)
 * @pspec: the #CParamSpec to set store a user data pointer
 * @quark: a #GQuark, naming the user data pointer
 * @data: (nullable): an opaque user data pointer
 ...
 *  be freed
 *
 * This function works like c_param_spec_set_qdata(), but in addition,
 * a `void (*destroy) (gpointer)` function may be
 * specified which is called with @data as argument when the @pspec is
 * finalized, or the data is being overwritten by a call to
 * c_param_spec_set_qdata() with the same @quark.
 */
void
c_param_spec_set_qdata_full (CParamSpec    *pspec,
			     GQuark         quark,
			     gpointer       data,
...
/**
 * c_param_spec_steal_qdata:
 * @pspec: the #CParamSpec to get a stored user data pointer from
 * @quark: a #GQuark, naming the user data pointer
 *
 * Gets back user data pointers stored via c_param_spec_set_qdata()
 ...
 */
gpointer
c_param_spec_steal_qdata (CParamSpec *pspec,
			  GQuark      quark)
{
  g_return_val_if_fail (C_IS_PARAM_SPEC (pspec), NULL);
...
/**
 * c_param_spec_get_redirect_target:
 * @pspec: a #CParamSpec
 *
 ...
 * of type #CParamSpecOverride. See g_object_class_override_property()
 ...
 */
CParamSpec*
c_param_spec_get_redirect_target (CParamSpec *pspec)
{
  GTypeInstance *inst = (GTypeInstance *)pspec;

  if (inst && inst->g_class && inst->g_class->g_type == C_TYPE_PARAM_OVERRIDE)
    return ((CParamSpecOverride*)pspec)->overridden;
  else
    return NULL;
}
 ...
 */
void
c_param_value_set_default (CParamSpec *pspec,
			   GValue     *value)
{
  g_return_if_fail (C_IS_PARAM_SPEC (pspec));
  ...
  if (G_VALUE_TYPE (value) == G_TYPE_INVALID)
    {
      g_value_init (value, C_PARAM_SPEC_VALUE_TYPE (pspec));
    }
  else
    {
  ...
  C_PARAM_SPEC_GET_CLASS (pspec)->value_set_default (pspec, value);
}

/**
 * c_param_value_defaults:
 * @pspec: a valid #CParamSpec
 ...
 */
gboolean
c_param_value_defaults (CParamSpec   *pspec,
			const GValue *value)
{
  GValue dflt_value = G_VALUE_INIT;
  gboolean defaults;

  g_return_val_if_fail (C_IS_PARAM_SPEC (pspec), FALSE);
  g_return_val_if_fail (G_IS_VALUE (value), FALSE);
  g_return_val_if_fail (PSPEC_APPLIES_TO_VALUE (pspec, value), FALSE);

  g_value_init (&dflt_value, C_PARAM_SPEC_VALUE_TYPE (pspec));
  C_PARAM_SPEC_GET_CLASS (pspec)->value_set_default (pspec, &dflt_value);
  defaults = C_PARAM_SPEC_GET_CLASS (pspec)->values_cmp (pspec, value, &dflt_value) == 0;
 ...
 * set out by @pspec. For example, a #CParamSpecInt might require
 ...
 */
gboolean
c_param_value_validate (CParamSpec *pspec,
			GValue     *value)
{
  g_return_val_if_fail (C_IS_PARAM_SPEC (pspec), FALSE);
  g_return_val_if_fail (G_IS_VALUE (value), FALSE);
  ...
  if (C_PARAM_SPEC_GET_CLASS (pspec)->value_validate)
    {
      GValue oval = *value;

      if (C_PARAM_SPEC_GET_CLASS (pspec)->value_validate (pspec, value) ||
 ...
 */
gboolean
c_param_value_is_valid (CParamSpec *pspec,
                        const GValue *value)
{
  CParamSpecClass *class;

  g_return_val_if_fail (C_IS_PARAM_SPEC (pspec), TRUE);
  g_return_val_if_fail (G_IS_VALUE (value), TRUE);
  g_return_val_if_fail (PSPEC_APPLIES_TO_VALUE (pspec, value), TRUE);

  class = C_PARAM_SPEC_GET_CLASS (pspec);
 ...
 */
gboolean
c_param_value_convert (CParamSpec   *pspec,
		       ...
		       gboolean	     strict_validation)
{
  ...
  if (g_value_transform (src_value, &tmp_value) &&
      (!c_param_value_validate (pspec, &tmp_value) || !strict_validation))
    {
 ...
 */
gint
c_param_values_cmp (CParamSpec   *pspec,
		    const GValue *value1,
		    const GValue *value2)
{
  gint cmp;
   ...
   *  1)  value1 > value2
   */
  g_return_val_if_fail (C_IS_PARAM_SPEC (pspec), 0);
  ...
  cmp = C_PARAM_SPEC_GET_CLASS (pspec)->values_cmp (pspec, value1, value2);
...
value_param_free_value (GValue *value)
{
  if (value->data[0].v_pointer)
    c_param_spec_unref (value->data[0].v_pointer);
}
			...
			GValue       *dest_value)
{
  if (src_value->data[0].v_pointer)
    dest_value->data[0].v_pointer = c_param_spec_ref (src_value->data[0].v_pointer);
  else
...
{
  if (src_value->data[0].v_pointer &&
      g_type_is_a (C_PARAM_SPEC_TYPE (dest_value->data[0].v_pointer), G_VALUE_TYPE (dest_value)))
    dest_value->data[0].v_pointer = c_param_spec_ref (src_value->data[0].v_pointer);
  else
    dest_value->data[0].v_pointer = NULL;
}
			   ...
			   guint        collect_flags)
{
  if (collect_values[0].v_pointer)
    {
      CParamSpec *param = collect_values[0].v_pointer;
	...
	return g_strconcat ("invalid unclassed param spec pointer for value type '",
			    ...
			    "'",
			    NULL);
      else if (!g_value_type_compatible (C_PARAM_SPEC_TYPE (param), G_VALUE_TYPE (value)))
	return g_strconcat ("invalid param spec type '",
			    C_PARAM_SPEC_TYPE_NAME (param),
			    "' for value type '",
			    G_VALUE_TYPE_NAME (value),
			    "'",
			    NULL);
      value->data[0].v_pointer = c_param_spec_ref (param);
    }
  else
    value->data[0].v_pointer = NULL;
...
value_param_lcopy_value (const GValue *value,
			 ...
			 guint         collect_flags)
{
  CParamSpec **param_p = collect_values[0].v_pointer;
  ...
  if (!value->data[0].v_pointer)
    ...
    *param_p = c_param_spec_ref (value->data[0].v_pointer);
 ...
 * CParamSpecPool:
 *
 * A #CParamSpecPool maintains a collection of #CParamSpecs which can be
 * quickly accessed by owner and name.
 *
 * The implementation of the #GObject property system uses such a pool to
 * store the #CParamSpecs of the properties all object types.
 */
struct _CParamSpecPool
{
  GMutex       mutex;
...
{
  const CParamSpec *key1 = key_spec_1;
  const CParamSpec *key2 = key_spec_2;
 ...
 *
 * Returns: (transfer full): a newly allocated #CParamSpecPool.
 */
CParamSpecPool*
c_param_spec_pool_new (gboolean type_prefixing)
{
  static GMutex init_mutex;
  CParamSpecPool *pool = g_new (CParamSpecPool, 1);
  ...
  pool->hash_table = g_hash_table_new_full (param_spec_pool_hash,
                                            ...
                                            (GDestroyNotify) c_param_spec_unref,
                                            NULL);
...
/**
 * c_param_spec_pool_free:
 * @pool: (transfer full): a #CParamSpecPool
 *
 * Frees the resources allocated by a #CParamSpecPool.
 *
 * Since: 2.80
 */
void
c_param_spec_pool_free (CParamSpecPool *pool)
{
...
/**
 * c_param_spec_pool_insert:
 * @pool: a #CParamSpecPool.
 * @pspec: (transfer none) (not nullable): the #CParamSpec to insert
 * @owner_type: a #GType identifying the owner of @pspec
 *
 * Inserts a #CParamSpec in the pool.
 */
void
c_param_spec_pool_insert (CParamSpecPool *pool,
			  CParamSpec     *pspec,
			  GType           owner_type)
{
  const gchar *p;
...
/**
 * c_param_spec_pool_remove:
 * @pool: a #CParamSpecPool
 * @pspec: (transfer none) (not nullable): the #CParamSpec to remove
 *
 * Removes a #CParamSpec from the pool.
 */
void
c_param_spec_pool_remove (CParamSpecPool *pool,
			  CParamSpec     *pspec)
{
  if (pool && pspec)
    {
...
/**
 * c_param_spec_pool_lookup:
 * @pool: a #CParamSpecPool
 * @param_name: the name to look for
 * @owner_type: the owner to look for
 * @walk_ancestors: If %TRUE, also try to find a #CParamSpec with @param_name
 *  owned by an ancestor of @owner_type.
 *
 * Looks up a #CParamSpec in the pool.
 *
 * Returns: (transfer none) (nullable): The found #CParamSpec, or %NULL if no
 * matching #CParamSpec was found.
 */
CParamSpec*
c_param_spec_pool_lookup (CParamSpecPool *pool,
			  const gchar    *param_name,
			  GType           owner_type,
			  gboolean        walk_ancestors)
{
...
/**
 * c_param_spec_pool_list_owned:
 * @pool: a #CParamSpecPool
 * @owner_type: the owner to look for
 *
 * Gets an #GList of all #CParamSpecs owned by @owner_type in
 * the pool.
 *
 * Returns: (transfer container) (element-type GObject.ParamSpec): a
 *          #GList of all #CParamSpecs owned by @owner_type in
 *          the pool#CParamSpecs.
 */
GList*
c_param_spec_pool_list_owned (CParamSpecPool *pool,
			      GType           owner_type)
{
  gpointer data[2];
...
pspec_compare_id (gconstpointer a,
		  gconstpointer b)
{
  const CParamSpec *pspec1 = a, *pspec2 = b;
...
should_list_pspec (CParamSpec *pspec,
                   GType      owner_type,
                   GHashTable *ht)
{
  CParamSpec *found;
   ...
   */
  if (c_param_spec_get_redirect_target (pspec))
    return FALSE;

  found = param_spec_ht_lookup (ht, pspec->name, owner_type, TRUE);
  if (found != pspec)
    {
      CParamSpec *redirect = c_param_spec_get_redirect_target (found);
      if (redirect != pspec)
        return FALSE;
    }
...
/**
 * c_param_spec_pool_list:
 * @pool: a #CParamSpecPool
 * @owner_type: the owner to look for
 * @n_pspecs_p: (out): return location for the length of the returned array
 *
 * Gets an array of all #CParamSpecs owned by @owner_type in
 * the pool.
 *
 * Returns: (array length=n_pspecs_p) (transfer container): a newly
 *          allocated array containing pointers to all #CParamSpecs
 *          owned by @owner_type in the pool
 */
CParamSpec**
c_param_spec_pool_list (CParamSpecPool *pool,
			GType           owner_type,
			guint          *n_pspecs_p)
{
  CParamSpec **pspecs, **p;
  ...
  pspecs = g_new (CParamSpec*, n_pspecs + 1);
  p = pspecs;
  for (i = 0; i < d; i++)
    {
...
static void
default_value_set_default (CParamSpec *pspec,
			   GValue     *value)
{
...
static gint
default_values_cmp (CParamSpec   *pspec,
		    const GValue *value1,
		    const GValue *value2)
{
 ...
 * @name: 0-terminated string used as the name of the new #CParamSpec type.
 * @pspec_info: The #CParamSpecTypeInfo for this #CParamSpec type.
 *
 * Registers @name as the name of a new static type derived
 * from %C_TYPE_PARAM.
 *
 * The type system uses the information contained in the #CParamSpecTypeInfo
 * structure pointed to by @info to manage the #CParamSpec type and its
 * instances.
 *
 * Returns: The new type identifier.
 */
GType
c_param_type_register_static (const gchar              *name,
			      const CParamSpecTypeInfo *pspec_info)
{
  GTypeInfo info = {
    sizeof (CParamSpecClass),      /* class_size */
    NULL,                          /* base_init */
    NULL,                          /* base_destroy */
  ...
  g_return_val_if_fail (pspec_info->instance_size >= sizeof (CParamSpec), 0);
  ...
  return g_type_register_static (C_TYPE_PARAM, name, &info, 0);
}

/**
 * c_value_set_param:
 * @value: a valid #GValue of type %C_TYPE_PARAM
 * @param: (nullable): the #CParamSpec to be set
 *
 * Set the contents of a %C_TYPE_PARAM #GValue to @param.
 */
void
c_value_set_param (GValue     *value,
		   CParamSpec *param)
{
  g_return_if_fail (C_VALUE_HOLDS_PARAM (value));
  if (param)
    g_return_if_fail (C_IS_PARAM_SPEC (param));

  if (value->data[0].v_pointer)
    c_param_spec_unref (value->data[0].v_pointer);
  value->data[0].v_pointer = param;
  if (value->data[0].v_pointer)
    c_param_spec_ref (value->data[0].v_pointer);
}

/**
 * c_value_set_param_take_ownership: (skip)
 * @value: a valid #GValue of type %C_TYPE_PARAM
 * @param: (nullable): the #CParamSpec to be set
 *
 * This is an internal function introduced mainly for C marshallers.
 *
 * Deprecated: 2.4: Use c_value_take_param() instead.
 */
void
c_value_set_param_take_ownership (GValue     *value,
				  CParamSpec *param)
{
  c_value_take_param (value, param);
}

/**
 * c_value_take_param: (skip)
 * @value: a valid #GValue of type %C_TYPE_PARAM
 * @param: (nullable): the #CParamSpec to be set
 *
 * Sets the contents of a %C_TYPE_PARAM #GValue to @param and takes
 ...
 */
void
c_value_take_param (GValue     *value,
		    CParamSpec *param)
{
  g_return_if_fail (C_VALUE_HOLDS_PARAM (value));
  if (param)
...
/**
 * c_value_get_param:
 * @value: a valid #GValue whose type is derived from %C_TYPE_PARAM
 *
 * Get the contents of a %C_TYPE_PARAM #GValue.
 *
 * Returns: (transfer none): #CParamSpec content of @value
 */
CParamSpec*
c_value_get_param (const GValue *value)
{
  g_return_val_if_fail (C_VALUE_HOLDS_PARAM (value), NULL);

  return value->data[0].v_pointer;
}

/**
 * c_value_dup_param: (skip)
 * @value: a valid #GValue whose type is derived from %C_TYPE_PARAM
 *
 * Get the contents of a %C_TYPE_PARAM #GValue, increasing its
 * reference count.
 *
 * Returns: (transfer full): #CParamSpec content of @value, should be
 *     unreferenced when no longer needed.
 */
CParamSpec*
c_value_dup_param (const GValue *value)
{
  g_return_val_if_fail (C_VALUE_HOLDS_PARAM (value), NULL);

  return value->data[0].v_pointer ? c_param_spec_ref (value->data[0].v_pointer) : NULL;
}

/**
 * c_param_spec_get_default_value:
 * @pspec: a #CParamSpec
 *
 ...
 **/
const GValue *
c_param_spec_get_default_value (CParamSpec *pspec)
{
  CParamSpecPrivate *priv = c_param_spec_get_private (pspec);
  ...
  if (g_once_init_enter_pointer (&priv->default_value.g_type))
    {
      ...
      c_param_value_set_default (pspec, &default_value);
 ...
 */
GQuark
c_param_spec_get_name_quark (CParamSpec *pspec)
{
  CParamSpecPrivate *priv = c_param_spec_get_private (pspec);
